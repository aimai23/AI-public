...
```bash
# TCP 22/443 の疎通確認
python3 tools/ping_hosts.py tools/hosts_example.txt --mode tcp --tcp-port 22 --tcp-port 443
```

```bash
# ICMP + TCP の同時確認
python3 tools/ping_hosts.py tools/hosts_example.txt --mode both --tcp-port 22
```

### 主なオプション

- `--mode`: 実行モード（`icmp` / `tcp` / `both`、既定: `icmp`）
- `-c, --count`: 1 対象あたりの送信回数（既定: `1`）
- `-t, --timeout`: 1 回あたりの応答待ち秒数（既定: `2`）
- `--tcp-port`: TCP 疎通確認ポート（複数指定可）
- `--tcp-timeout`: TCP connect タイムアウト秒（既定: `2.0`）
- `-w, --workers`: 並列ワーカー数（既定: `20`）
- `--show-fail-output`: 失敗対象のみ詳細メッセージを標準出力にも表示（通常はログファイルのみ）
- `--show-all-output`: 全対象の詳細メッセージを標準出力にも表示（通常はログファイルのみ）
